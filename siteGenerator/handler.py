...
def download_from_s3(s3_path,local_path):
    ...
    run_command('aws/aws s3 sync s3://{0} {1}'.format(s3_path,local_path))
...
def build_hugo(source_dir, destination_dir,debug=False):
    ...
    run_command("hugo/hugo -s {0} -d {1}".format(source_dir,destination_dir))
...
def upload_to_s3(local_path,s3_path):
    ...
    run_command('aws/aws s3 rm s3://{0} --recursive'.format(s3_path))
    run_command('aws/aws s3 sync {0} s3://{1}'.format(local_path,s3_path))
    run_command('aws/aws s3 ls {0}'.format(s3_path))
