...
it("/logout", async () => {
  ...
  expect(res.multiValueHeaders.location).to.include('https://AuthDomain/logout?client_id=UserPoolClientId&logout_uri=https://www.always-onward.com')
...
it("/refresh No Refresh Token", async () => {
  ...
  expect(res.body).to.equal('{"login":false,"redirect_url":"https://AuthDomain/login?client_id=UserPoolClientId&response_type=code&scope=email+openid+phone+profile&redirect_uri=https://www.always-onward.com/api/auth/callback"}')
});
...
it("/callback ", async () => {
  ...
  expect(res.multiValueHeaders['set-cookie'].join()).to.include('refresh_token')
  expect(res.multiValueHeaders['set-cookie'].join()).to.include('access_token')
  expect(res.multiValueHeaders.location).to.include('https://www.always-onward.com')
});
